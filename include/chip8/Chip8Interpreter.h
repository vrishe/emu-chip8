...
namespace chip8 {
	...
	class Interpreter : protected TimerListener {
	public:
		enum : word {
			STACK_DEPTH				= 0x10,
		...
		std::auto_ptr<Timer> timerDelayPulse;
		std::auto_ptr<Timer> timerSoundPulse;

		Interpreter(TimerFactory timerFactory, HardwareHandler *hardwareHandler = nullptr);
		...
		void onTick(int timerId, size_t tick);
		...
		public:
			...
			word getTimerDelay() const {
				return soc->timerDelay;
			}
			word getTimerSound() const {
				return soc->timerSound;
			}
